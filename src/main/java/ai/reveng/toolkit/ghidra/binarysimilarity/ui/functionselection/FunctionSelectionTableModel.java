...
public class FunctionSelectionTableModel extends ThreadedTableModelStub<FunctionRowObject> {
     ...
     * External functions are excluded from the list entirely.
    ...
    public void initForProgram(Program program) {
        ...
        if (program != null) {
            program.getFunctionManager().getFunctions(true).forEach(function -> {
                // Skip external functions - they are not relevant for analysis
    ...
    protected TableColumnDescriptor<FunctionRowObject> createTableColumnDescriptor() {
        ...
        addRowToDescriptor(descriptor, "Size", Long.class, FunctionRowObject::getSize);
        return descriptor;
    }

    @Override
    public boolean isCellEditable(int rowIndex, int columnIndex) {
        return columnIndex == SELECT;
    }
    ...
    /**
     * Select all functions in the table.
     */
    ...
    /**
     * Select all non-thunk functions.
     * Thunk functions will be deselected.
