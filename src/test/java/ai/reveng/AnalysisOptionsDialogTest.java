...
public class AnalysisOptionsDialogTest extends RevEngMockableHeadedIntegrationTest {
    ...
    public void testGetOptionsFromUIIncludesSelectedFunctions() throws Exception {
        ...
        // All functions should be sent as boundaries (not just selected ones)
        int totalCount = functionSelectionPanel.getTotalFunctionCount();
        assertEquals("Function boundaries count should match total functions",
                totalCount, boundaries.size());
        ...
        // All boundaries should have includeInAnalysis=true since all functions are selected
        for (var boundary : boundaries) {
            assertTrue("All selected boundaries should have includeInAnalysis=true",
                    boundary.getIncludeInAnalysis());
        }
